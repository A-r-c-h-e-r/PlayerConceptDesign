    ...
    public static class MusicTemplate
    {
        public static Grid TemplateCover(Grid grid, BitmapImage Cover)
        {
            var CoverShadow = new Border
            {
                ...
                Opacity = 0.9,
                Margin = new Thickness(ApplicationSettings.Default.SizeCover / 45)
            };
            ...
            var CoverSong = new Border
            {
                ...
                CornerRadius = new CornerRadius(ApplicationSettings.Default.SizeCover / 20),
                Background = new ImageBrush()
                {
                    ImageSource = Cover
                },
        ...
        public static Border TemplateActiveCoverImagePlayPause(Border ActiveCover)
        {
            return new Border
            {
                ...
                Opacity = 0.8,
                Margin = new Thickness(0,0,0, (ApplicationSettings.Default.SizeCover /2 )-2.5),
        ...
        public static Border TemplateActiveCoverImageLike(Border ActiveCover, string file, int index)
        {
            ...
            for (int i = 0; i < ApplicationSettings.Default.FilesLike.Count; i++)
                if (ApplicationSettings.Default.FilesLike[i] == file) { LikeStatus = "like_active"; break; }
        ...
        public static Grid TemplateTile(Grid grid, Grid cover, string name)
        {
            ...
            double test = 0;
            if (ApplicationSettings.Default.SizeCover <= 120) test = 4;
            else if (ApplicationSettings.Default.SizeCover <= 130) test = 3;
            else if (ApplicationSettings.Default.SizeCover <= 140) test = 2;
            else if (ApplicationSettings.Default.SizeCover <= 150) test = 1;
            Label labelName = new Label
            {
                Content = name,
                VerticalAlignment = VerticalAlignment.Bottom,
                HorizontalAlignment = HorizontalAlignment.Left,
                Margin = new Thickness(ApplicationSettings.Default.SizeCover / 10, 0, ApplicationSettings.Default.SizeCover / 7.9, ApplicationSettings.Default.SizeCover / 11 - test),
                FontSize = ApplicationSettings.Default.SizeCover / 10 + test,
            ...
            Label labelName2 = new Label
            {
                ...
                VerticalAlignment = VerticalAlignment.Bottom,
                HorizontalAlignment = HorizontalAlignment.Right,
                Margin = new Thickness(0, 0, ApplicationSettings.Default.SizeCover / 20, ApplicationSettings.Default.SizeCover / 11 - test),
                FontSize = ApplicationSettings.Default.SizeCover / 10 + test,
                Foreground = SetColorTheme.colorsTheme.ColorForeground,
            };
